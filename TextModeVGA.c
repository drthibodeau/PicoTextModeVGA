...
void PIXEL_Pin_Init(PIO pio, uint sm, uint offset, uint pin) {
    ...
    Standard_PIO_Config(pio, sm, offset, pin, 1, &config);                                                      // set standard pin init commands
    ...
    // Even though the shift out direction is to left, a full 32 bit load does not have bits mirrored 
...
void HSYNC_Pin_Init(PIO pio, uint sm, uint offset, uint pin) {
    ...
    // PIO config structure
    // xxx_get_default_config() function is a helper function generated by the PIO assembler.  
    // offset contains the memory address where the compiled program begins. Set by pio_claim_free_sm_and_add_program_for_gpi 
    pio_sm_config config = SYNC_program_get_default_config(offset);

    // common pin init commands
    Standard_PIO_Config(pio, sm, offset, pin, 1, &config); 

    float div = 4;
    sm_config_set_clkdiv(&config, div);
    
    // initializes a PIO (Programmable Input/Output) state machine with a specific program and configuration.
    // offset: This is the memory address (offset) within the PIO's instruction memory where the compiled PIO program is loaded. 
    // offset could be a different location within program indicated by public label directive 
    pio_sm_init(pio, sm, offset+SYNC_offset_hsync_start, &config);
...
void VSYNC_Pin_Init(PIO pio, uint sm, uint offset, uint pin) {
    ...
    // common pin init commands
    Standard_PIO_Config(pio, sm, offset, pin, 2, &config); 
    ...
    // adding handler for PIO0_IRQ_0
    irq_set_exclusive_handler(PIO0_IRQ_0, VSYNC_IRQ_handler);
    ...
    // offset: This is the memory address (offset) within the PIO's instruction memory where the compiled PIO program is loaded. 
    // offset could be a different location within program indicated by public label directive 
    pio_sm_init(pio, sm, offset+SYNC_offset_vsync_start, &config);
    
    // auto pull with 32 bit threshold
    pio->sm[sm].shiftctrl = (1u << PIO_SM0_SHIFTCTRL_AUTOPULL_LSB);

    // init line counts for vertical regions. Sub 1 for the 0th iteration in PIOASM
    pio->txf[sm] = V_ACTIVE_LINES-1;
...
void Standard_PIO_Config(PIO pio, uint sm, uint offset, uint pin, uint num_pins, pio_sm_config* config_ptr) {
        ...
        // connects GPIO "pin" to the PIO
        for(int i=0; i<num_pins; i++) pio_gpio_init(pio, pin+i);
       
        // sets one or more consecutive pin directions
        // sm is index of the PIO state machine (0, 1, 2, or 3)
        // pin is the starting GPIO pin number. Fourth input is number of consecutive pins starting from this pin.
        // last input is direction: true for outputs, false for input
        pio_sm_set_consecutive_pindirs(pio, sm, pin, num_pins, true);
        
        // modifies PIO state machine config to specify which pin(s) the out and set instruction will affect.
        // passing the address of the config structure so that the function can modify it directly.
        // GPIO pin number that you want the set instruction to affect. Last input is the number of consecutive pins to affect. 
        sm_config_set_set_pins(config_ptr, pin, num_pins);
